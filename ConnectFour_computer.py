...
import functions.CheckWin as CheckWin
import functions.GetComputerMove as GetComputerMove
...
def onClick(r, c):
    ...
    move = GetComputerMove(scoreBoard, turn)
    if type(move) == int:
        for row in range(5, -1, -1):
            if scoreBoard[row][move] == ' ':
                scoreBoard[row][move] = turns[turn]
                displayBoard[row][move].configure(fg_color=turns[turn])
                moves += 1
                break
    else: 
        gameText.configure(text = 'Sadly enough, it ended in a draw..')
        stopGame = True
        return
